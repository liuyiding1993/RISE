...
function showSearchResult(data) {
    ...
    $("#result_stat").html("Time cost: "+ data["runtime"] +" seconds.");
    if(!data['valid']){
        ...
        var long_side = Math.max(data['query']['height'], data['query']['width']);
        var zoom_level = (15 - long_side) / 14.0 * 4.0 + 13 - Math.log(long_side);
        console.log(zoom_level);
        zoom_level = Math.max(12, Math.round(zoom_level));
        console.log(zoom_level);
...
function showPoiInside(result_pois) {
    showPoiInsideDefault(result_pois);
    showPoiInsideCategory(result_pois);
}


function erasePoiInside() {
    $("#result_list").empty();
    $("#result_list_by_category").empty();
}


function hideRankButtons() {
    $("#default_rank").hide();
    $("#category_rank").hide();
}
...
function showPoiInsideCategory(pois) {
    for (var i = 0; i < pois.length; i++) {
        ...
        for (var j = 0; j < poi['categories'].length; j++) {
            ...
            if (category_pois[category] == undefined) {
                category_pois[category] = [];
                category_markers[category] = [];
            }
            category_pois[category].push(poi);
            addMarker(poi['poi_name'], poi['lat'], poi['lng'], colors[category_id[category] % colors.length], category_markers[category]);
        }
    }
    ...
    $('.my_accordion .acd_head').click(function() {
        ...
        if ($(this).attr("toggled") == 'true') {
            for (var i = 0; i < category_markers[category].length; i++)
                category_markers[category][i].setVisible(true);
            $(this).attr("toggled", 'false');
        }
        else {
            for (var i = 0; i < category_markers[category].length; i++)
                category_markers[category][i].setVisible(false);
...
function addCategory(categorized_pois) {
    ...
    accordion += "<h3 class='acd_head'" + " name=\"" + category + "\" toggled='true' style='color: " + colors[category_id[category]] + "; cursor: pointer'>";
    accordion += category;
    accordion += "&nbsp;";
    accordion += "<span class='badge'>" + pois.length + "</span>";
    accordion += "</h3>";
    accordion += "<div>";
...
$("#default_rank").on('click', function() {
    for (var category in category_markers)
        for (var i = 0; i < category_markers[category].length; i++)
            category_markers[category][i].setVisible(true);
    $("#result_list_by_category").hide();
    $("#result_list").show();
